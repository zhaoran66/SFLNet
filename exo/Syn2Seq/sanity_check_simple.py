"""
Simple Syn2Seq Baseline Sanity Check
Directly sample and compare
"""
import os
...
def main():
    print("=" * 70)
    print("SYN2SEQ BASELINE - SIMPLE SANITY CHECK")
    print("=" * 70)
    ...
    num_samples = 5
    os.makedirs("./sanity_simple", exist_ok=True)
    ...
    for sample_idx in range(num_samples):
        ...
        with torch.no_grad():
            ...
            print("Sampling Syn2Seq (in pixel space)...")
            history_frames = exo_video[:, :, -num_history:]
            
            x = torch.randn(1, 3, num_history + num_interp, 128, 128, device=device)
            x[:, :, :num_history] = history_frames[:1]
            ...
            for t in tqdm(reversed(range(diffusion.num_timesteps)), desc="Diffusion", leave=False):
                ...
                x_noisy_history = sqrt_alpha_bar_t * history_frames[:1] + sqrt_one_minus_alpha_bar_t * torch.randn_like(history_frames[:1])
            ...
            gen_interp = x[:, :, -num_interp:]
            gt_interp = interp_frames[:1]
            ...
            for i in range(num_interp):
                ...
                vutils.save_image(
                    comparison,
                    f"./sanity_simple/sample{sample_idx}_frame{i}_psnr{psnr:.2f}_ssim{ssim:.4f}.png",
                    normalize=False,
                )
            ...
            print(f"\nVisualization saved: ./sanity_simple/sample{sample_idx}_frame*.png")
    
    print(f"\n{'='*70}")
    print("SYN2SEQ BASELINE FINAL RESULTS")
    print(f"{'='*70}")
    ...
    print(f"{'='*70}")
    print(f"\nFASR Results (from ../new/sanity_check/):")
    print(f"  PSNR: 17.52 dB")
    print(f"  SSIM: 0.2757")
    print(f"  Temporal Consistency: 0.018998")
    print(f"  Sharpness Ratio: 21.13%")
    print(f"\nCompare these values:")
    print(f"  1. Is FASR sharper than Syn2Seq?")
    print(f"  2. Is FASR more temporally stable?")
    print(f"{'='*70}")
