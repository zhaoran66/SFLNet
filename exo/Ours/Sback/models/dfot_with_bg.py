...
class DiffusionForcingTransformerWithBG(nn.Module):
    """
    Syn2Seq + Background Processing (EXACT SAME as Ours/new)
    
    Components:
    1. Soft Spectral Decomposition (DCT + Gaussian masks)
    2. Structure-Weighted Asymmetric Loss (foreground + background)
    3. Gaussian Pose Masking
    """
    def __init__(
        self,
        in_channels: int = 3,
    ...
    def compute_losses(
        ...
        full_video_sequence = torch.cat([exo_video[:, :, -num_history:], ego_video[:, :, :4]], dim=2)
        
        t = torch.randint(0, diffusion.num_timesteps, (B,), device=exo_video.device)
        noise = torch.randn_like(full_video_sequence)
        x_t = diffusion.q_sample(full_video_sequence, t, noise)
        ...
        pred_video = x_t + model_output
        target_video = full_video_sequence
