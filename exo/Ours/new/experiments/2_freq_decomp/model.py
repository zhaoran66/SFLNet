...
class SoftSpectralDecomposition(nn.Module):
    def __init__(self, freq_size: Tuple[int, int] = (8, 8), feat_dim: int = 384, sigma: float = 0.5):
        super().__init__()
...
class HandPoseFreqDecomp(nn.Module):
    def __init__(self, in_channels=3, num_frames=8, hidden_dim=256, num_pose_params=51, use_freq_decomp=True):
        super().__init__()
