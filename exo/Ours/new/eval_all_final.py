#!/usr/bin/env python
"""
Final Evaluation for All 3 Hand Pose Methods:
1. Baseline
2. freq_decomp
3. latent_only
"""
...
@torch.no_grad()
def evaluate_method(method_name, model_class, val_loader, device, args):
    print(f"\n{'=' * 80}")
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate All Hand Pose Methods')
    ...
    print("=" * 80)
    print("Hand Pose Estimation - Final Evaluation of All Methods")
    print("=" * 80)
    ...
    for method_name, model_class in methods:
        results = evaluate_method(method_name, model_class, val_loader, device, args)
        if results is not None:
            all_results[method_name] = results
    
    if len(all_results) > 1:
        print(f"\n{'=' * 80}")
        print(f"FINAL COMPARISON: MSE (Lower is Better)")
        print(f"{'=' * 80}")
        ...
        print(f"\nRankings by MSE:")
        sorted_methods = sorted(all_results.keys(), key=lambda m: all_results[m]['mse_mean'])
        for i, method in enumerate(sorted_methods):
            print(f"  #{i+1}: {method}")
    
    results_path = os.path.join(args.output_dir, 'final_comparison.npy')
