"""
Hand Pose Estimation Models (21 joints, 51 DOF MANO parameters)
...
Fix: Use LayerNorm instead of BatchNorm to handle batch size = 1
"""
import torch
...
class HandPoseLatentOnly(nn.Module):
    """
    Latent-only: SD-VAE Latent + MLP Regressor
...
class HandPoseOurs(nn.Module):
    """
    Ours: DINO Feature + Frequency Decomposition + Pose Regressor
...
class HandPoseMethodC(nn.Module):
    """
    Method C: SD-VAE Latent + Frequency Decomposition + Pose Regressor
    Input: Exo view video latent (B, 4, T, 16, 16)
    Output: Hand pose parameters (B*T, 51)
    """
    def __init__(
