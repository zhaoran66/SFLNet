"""
Evaluation Script for 4 Hand Pose Estimation Methods
Compares all 4 methods on the same validation set
"""
import os
import sys
import argparse
...
@torch.no_grad()
def evaluate_model(model, data_loader, device, method_name, vae=None, dino_extractor=None):
    ...
    for batch in tqdm(data_loader, desc=f"Evaluating {method_name}"):
        ...
        if vae is not None:
            z = vae.encode(exo_video)
            pose_pred = model(z)
        elif dino_extractor is not None:
            feat = dino_extractor(exo_video)
            pose_pred = model(feat)
        else:
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate 4 Hand Pose Estimation Methods')
    ...
    print("="*70)
    print("Hand Pose Estimation - Evaluation of 4 Methods")
    print("="*70)
    ...
    for method_name, method_desc in methods:
        ...
        if method_name in ['latent_only', 'latent_freq']:
            if vae is None:
        ...
        if method_name == 'freq_decomp':
            if dino_extractor is None:
                dino_extractor = DINOv2FeatureExtractor(output_size=(32, 32))
                dino_extractor.to(device)
                dino_extractor.eval()
                print("Loaded DINOv2 feature extractor")
        ...
        if method_name == 'baseline':
            ...
            metrics, _, _ = evaluate_model(model, val_loader, device, method_name, vae=None, dino_extractor=None)
        elif method_name == 'freq_decomp':
            model = load_model(
                checkpoint_path,
                HandPoseOurs,
                {'feat_dim': 384, 'num_frames': config.data.num_frames, 'hidden_dim': 256, 'num_pose_params': 51, 'use_freq_decomp': True},
                device
            )
            metrics, _, _ = evaluate_model(model, val_loader, device, method_name, vae=None, dino_extractor=dino_extractor)
        elif method_name == 'latent_only':
            model = load_model(
                checkpoint_path,
                HandPoseLatentOnly,
                {'in_channels': 4, 'num_frames': config.data.num_frames, 'hidden_dim': 256, 'num_pose_params': 51},
                device
            )
            metrics, _, _ = evaluate_model(model, val_loader, device, method_name, vae=vae, dino_extractor=None)
        elif method_name == 'latent_freq':
            model = load_model(
                checkpoint_path,
                HandPoseMethodC,
                {'latent_dim': 4, 'num_frames': config.data.num_frames, 'hidden_dim': 256, 'num_pose_params': 51, 'use_freq_decomp': True},
                device
            )
            metrics, _, _ = evaluate_model(model, val_loader, device, method_name, vae=vae, dino_extractor=None)
