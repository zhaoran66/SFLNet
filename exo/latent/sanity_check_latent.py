"""
Sanity Check for Syn2Seq + Latent (WITHOUT Frequency Routing)
"""
import os
...
def main():
    print("=" * 70)
    print("SYN2SEQ + LATENT SANITY CHECK (NO FREQ ROUTING)")
    print("=" * 70)
    ...
    vae = FrameVAE(latent_dim=config.model.hidden_size)
    ...
    for sample_idx in range(num_samples):
        ...
        with torch.no_grad():
            full_sequence, interp_frames = interpolator(exo_video, ego_video, vae)
            pose_cond = torch.cat([exo_pose[:, -1:], ego_pose[:, :1]], dim=1)
            
            print("Sampling Syn2Seq + Latent (VAE feature space)...")
            B = exo_video.shape[0]
            
            feat_history = vae.encode(exo_video[:, :, -num_history:].permute(0, 2, 1, 3, 4).reshape(-1, 3, 128, 128))
            feat_history = feat_history.reshape(B, num_history, -1).permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1)
            feat_dim = feat_history.shape[1]
            
            x = torch.randn(B, feat_dim, num_history + num_interp, 1, 1, device=device)
            ...
            pred_feat = x[:, :, -num_interp:].squeeze(-1).squeeze(-1).permute(0, 2, 1).reshape(-1, feat_dim)
            gen_interp = vae.decode(pred_feat).reshape(B, num_interp, 3, 128, 128).permute(0, 2, 1, 3, 4)
