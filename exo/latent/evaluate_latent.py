"""
Evaluation for the latent version:
  - Encodes RGB to latent space
  - Runs diffusion in latent space
  - Decodes generated latent back to RGB
  - Computes PSNR / SSIM / LPIPS against GT
"""
import os
import sys
import gc
...
class LatentEvaluator:
    def __init__(self, vae, model, diffusion, interpolator, val_loader, config):
        self.vae = vae
        self.model = model
        ...
        for m in [vae, model, diffusion, interpolator]:
            m.to(self.device)
            m.eval()
    ...
    def evaluate(self):
        ...
        print("Evaluation Results (on interpolated frames, pixel space):")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./outputs_latent", help="Output directory")
    ...
    evaluator = LatentEvaluator(vae, model, diffusion, interpolator, val_loader, config)
    ...
    print(f"Results saved to {os.path.join(config.output_dir, 'eval_results.txt')}")
