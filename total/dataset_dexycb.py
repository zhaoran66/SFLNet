...
class DexYCBMultiViewReal(Dataset):
    ...
    def _load_joints_3d(self, data_idx: int, wrist_center: bool = True) -> torch.Tensor:
        try:
            sample = self.ds[data_idx]
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if HAS_DEXYCB:
            for frame_data in seq['frames']:
                ...
                kp = self._load_joints_3d(ego_data_idx, wrist_center=True)
                ego_keypoints.append(kp)
                
                # Load EXO 3D joints (first exo view) for geometry baseline
                exo_data_idx = frame_data[self.exo_views[0]]
                exo_kp = self._load_joints_3d(exo_data_idx, wrist_center=False)
                exo_keypoints.append(exo_kp)
        else:
